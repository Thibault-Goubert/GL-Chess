...
namespace Echecs.IHM
{
    public partial class FenetreDeJeu : Form, IEvenements
    {
        ...
        public void ActualiserCaptures(List<InfoPiece> pieces)
        {
            ...
            if(pieces.Count == 0)
            {
                for(int idx=0; idx<captures_noirs.Length; idx++)
                {
                    captures_noirs[idx].Image = null;
                    captures_blancs[idx].Image = null;
                }
            }
        ...
        public void ActualiserListMove(string[] move)
        {
            if(move != null)
            {
                lvwMoveHistory.Items.Add(new ListViewItem(move));
            }
            else
            {
                foreach (ListViewItem item in lvwMoveHistory.Items)
                {
                    lvwMoveHistory.Items.Remove(item);
                }
            }
        }
